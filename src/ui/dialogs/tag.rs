...
pub fn show(
    ...
    let mut keep_open = state.dialogs.tag.show_create_tag_dialog;
    ...
    egui::Window::new("Create Tag")
        ...
        .open(&mut keep_open)
        .show(ctx, |ui| {
            ui.label(format!("Current branch: {}", state.repo.branch));
            ui.add_space(6.0);
            ui.label("Tag name");
            let response = ui.add_enabled(
                !create_tag_busy,
                egui::TextEdit::singleline(&mut state.dialogs.tag.new_tag_name)
                    .desired_width(260.0)
                    .hint_text("v1.0.0.0"),
            );
            if state.dialogs.tag.focus_new_tag_name_requested {
                response.request_focus();
                state.dialogs.tag.focus_new_tag_name_requested = false;
            }
            let can_submit = can_create_tag && !state.dialogs.tag.new_tag_name.trim().is_empty();

            if response.lost_focus()
                && ui.input(|input| input.key_pressed(egui::Key::Enter))
                && can_submit
            {
                submit_tag = Some(state.dialogs.tag.new_tag_name.trim().to_string());
            }

            ui.add_space(8.0);
            if can_create_tag {
                if state.repo.has_origin_remote {
            ...
            } else if state.repo.has_github_https_origin && !github_auth_available {
