...
fn draw_tab(
    ...
    let tab_height = text_size.y.max(CLOSE_SIZE) + PAD_Y * 2.0;
    let (rect, response) =
    ...
    // Clone so the painter borrow does not conflict with the later `ui.put`.
    ...
    let close_response = ui.put(
        close_rect,
        egui::Button::new(egui::RichText::new("×").size(CLOSE_SIZE))
            .frame(false)
            .small(),
    );
